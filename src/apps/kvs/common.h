...
static int
init_signal_handling(uv_loop_t *loop)
{
  int error;
  uv_signal_t *sig = (uv_signal_t *)malloc(sizeof(*sig));
  uv_prepare_t *prepare = (uv_prepare_t *)malloc(sizeof(*prepare));
  assert(sig != NULL || prepare != NULL);

  error = uv_signal_init(loop, sig);
  assert(error == 0);

  error = uv_signal_start(sig, on_sig, SIGINT);
  assert(error == 0);

  error = uv_prepare_init(loop, prepare);
  assert(error == 0);

  error = uv_prepare_start(prepare, on_prepare);
  assert(error == 0);
...
try_connect(uv_timer_t *handle)
{
  ...
  if (error) {
    printf("Could not connect to proxy. Retrying...\n");
    close(sock);
    return;
  }
